...
def ink_annotation(ink, rect, contents=""):
    """ A quick test of Ink annotations, which contain single line drawings.
    Unfortunately, these are drawn on the document with a text annotation,
    instead of having the drawing be the annotation.
    """
    
    return DictionaryObject({   NameObject('/Rect'): float_array(rect),
                                NameObject('/Type'): NameObject('/Annot'),
                                NameObject('/Subtype'): NameObject('/Ink'),
                                NameObject('/Contents'): TextStringObject(contents),
                                NameObject('/InkList'): ArrayObject([float_array(ink)]),
                                NameObject('/C'): float_array([0,0,0])
                            })
